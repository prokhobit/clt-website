(function () {
    ...
    var DEFAULTS = {
      ...
      lenis: {
        ...
        prevent: function (node) {
          return !!(node && node.closest && node.closest('.clt-dialog, .clt-dialog__scroll, [data-lenis-prevent]'));
        },
      },
    };
    ...
    function ensureLenisCss() {
      ...
      style.textContent = ['html.lenis, html.lenis body{height:auto;}', '.lenis.lenis-smooth{scroll-behavior:auto!important;}', '.lenis.lenis-smooth [data-lenis-prevent]{overscroll-behavior:contain;}', '.lenis.lenis-stopped{overflow:clip;}', '.lenis.lenis-stopped .clt-dialog[open]{overflow:hidden;}', '.lenis.lenis-smooth iframe{pointer-events:none;}'].join('');
    ...
    function ensureDialogCss() {
      ...
      style.textContent = ['.clt-dialog{max-height:min(92dvh,calc(100dvh - 2rem));overflow:hidden;}', '.clt-dialog[open]{overscroll-behavior:contain;}', '.clt-dialog.is-closing{pointer-events:none;}', '.clt-dialog::backdrop{opacity:0;background:rgb(0 0 0 / 0.72);transition:opacity .46s cubic-bezier(.16,1,.3,1);}', '.clt-dialog.is-open::backdrop{opacity:1;}', '.clt-dialog.is-closing::backdrop{opacity:0;}', '.clt-dialog__body{min-height:0;}', '.clt-dialog__scroll{max-height:min(62dvh,calc(100dvh - 15rem));overflow:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;}', '.clt-dialog__actions{flex-shrink:0;}', 'html.clt-dialog-is-open,body.clt-dialog-is-open{overflow:hidden;overscroll-behavior:contain;}', 'body.clt-dialog-is-fixed{position:fixed;left:0;right:0;width:100%;}'].join('');
      document.head.appendChild(style);
    }
    ...
    function initScroll() {
      ...
      if (gsap && Lenis && !env.isTouch && !env.reducedMotion) {
        ...
        CLT.startScroll = function () {
          if (CLT.lenis && typeof CLT.lenis.start === 'function') CLT.lenis.start();
        };
      ...
      if (!CLT.lenis) {
        CLT.scrollTo = function (target) {
          if (typeof target === 'number') window.scrollTo(0, target);
          else if (typeof target === 'string') {
            var node = document.querySelector(target);
            if (node && typeof node.scrollIntoView === 'function') node.scrollIntoView();
          } else if (target && typeof target.scrollIntoView === 'function') target.scrollIntoView();
        };
      }
    ...
    function initDialogs(root) {
      ...
      function setPageDialogState(isOpen) {
        ...
        if (document.body) document.body.classList.toggle('clt-dialog-is-open', isOpen);
        if (isOpen) {
          CLT.stopScroll();
      ...
      function getFocusTarget(d) {
        return d.querySelector('[autofocus], [data-dialog-focus], [data-close-dialog], .clt-button, [href], [tabindex]:not([tabindex="-1"]), [contenteditable="true"]');
      }

      function getScrollY() {
        if (CLT.lenis && typeof CLT.lenis.scroll === 'number') return CLT.lenis.scroll;
      ...
      function lockPageScroll(y) {
        ...
        var computedPadding = parseFloat(window.getComputedStyle(body).paddingRight) || 0;
      ...
      function getDialogScroller(target, d) {
        ...
        var scroller = target.closest('.clt-dialog__scroll, [data-lenis-prevent]');
      ...
      function resetDialogMotion(d) {
        ...
        gsap.killTweensOf([d, targets.close, targets.body, targets.actions, targets.scroller]);
        gsap.set([d, targets.close, targets.body, targets.actions, targets.scroller].filter(Boolean), { clearProps: 'transform,opacity,visibility,filter,willChange' });
      }
      ...
      function animateDialogOpen(d) {
        ...
        gsap.killTweensOf([d, targets.body, targets.actions, targets.close, targets.scroller].filter(Boolean));
        gsap.set(d, {
          autoAlpha: 0,
          y: 30,
          scaleX: 0.958,
      ...
      function isNativeDialogControl(node) {
        return !!(node && ((window.HTMLButtonElement && node instanceof window.HTMLButtonElement) || (window.HTMLAnchorElement && node instanceof window.HTMLAnchorElement) || (window.HTMLInputElement && node instanceof window.HTMLInputElement)));
      }
      ...
      function prepareDialogControl(node) {
        ...
        if (window.HTMLButtonElement && node instanceof window.HTMLButtonElement && !node.hasAttribute('type')) {
      ...
      function handleCloseCommand(e, control) {
        ...
        var d = (control.closest && control.closest('.clt-dialog')) || getDialog(control.getAttribute('data-close-dialog'));
      ...
      CLT.dialogs.close = function (target, value) {
        closeDialog(typeof target === 'string' ? getDialog(target) : target, value);
      };
      ...
      $all('.clt-dialog', root).forEach(function (d) {
        ...
        d.addEventListener('click', function (e) {
          if (e.target === d && !d.hasAttribute('data-dialog-static')) closeDialog(d, 'dismiss');
        });
      ...
      if (!CLT.__dialogDelegationReady) {
        ...
        document.addEventListener(
          ...
          function (e) {
            ...
            var openControl = commandControlFromEvent(e, '[data-open-dialog]');
            if (openControl && !isNativeDialogControl(openControl)) handleOpenCommand(e, openControl);
          },
          true,
        );
        ...
        document.addEventListener(
          ...
          function (e) {
            ...
            if (submitter && submitter.closest && submitter.closest('[data-open-dialog], [data-close-dialog]')) {
              stopCommandEvent(e);
            }
          },
          true,
    ...
    function initTabs(root) {
      $all('[data-clt-tabs]', root || document).forEach(function (tabs) {
        ...
        var scope = (tabs.closest && tabs.closest('[data-clt-tabs-scope]')) || tabs.parentElement || document;
        var panels = $all('[data-panel]', scope);
        if (!panels.length) panels = $all('[data-panel]');

        function panelMatches(panel, id, controls) {
          return (controls && panel.id === controls) || (id && panel.getAttribute('data-panel') === id);
        }
        ...
        btns.forEach(function (t, index) {
          ...
          t.addEventListener('keydown', function (e) {
            ...
            var next = index;
            if (key === 'ArrowRight' || key === 'ArrowDown') next = (index + 1) % btns.length;
            if (key === 'ArrowLeft' || key === 'ArrowUp') next = (index - 1 + btns.length) % btns.length;
            if (key === 'Home') next = 0;
      ...
      $all('[data-clt-toggle]', root || document).forEach(function (tg) {
        ...
        opts.forEach(function (o, index) {
          ...
          o.addEventListener('keydown', function (e) {
            ...
            var next = index;
            if (key === 'ArrowRight' || key === 'ArrowDown') next = (index + 1) % opts.length;
            if (key === 'ArrowLeft' || key === 'ArrowUp') next = (index - 1 + opts.length) % opts.length;
            if (key === 'Home') next = 0;
    ...
    function initSectionNav(root) {
      $all('[data-clt-sectionnav]', root || document).forEach(function (nav) {
        ...
        function update() {
          ...
          var y = CLT.lenis && typeof CLT.lenis.scroll === 'number' ? CLT.lenis.scroll : window.scrollY || 0;
    ...
    function initPanelOrb() {
      ...
      var active = null,
        ...
        cmx = 50,
        cmy = 50;
      $all('.clt-panel:not(.is-carved):not(.is-marquee):not(.is-noir):not(.is-frame)').forEach(function (p) {
    ...
    function initDust() {
      ...
      var density = reduced ? 0.35 : env.isTouch || env.isSmallViewport ? 0.7 : config.density;
      ...
      addTick(function () {
        ...
        var velocity = CLT.lenis && typeof CLT.lenis.velocity === 'number' ? CLT.lenis.velocity * 1000 : scrollDelta * 60;
        lastScroll = scroll;
        ...
        for (var i = 0; i < stars.length; i++) {
          ...
          var stretch = clamp(1, 1 + 0.72 * warp, 1 + (Math.abs(velocity) / 4200) * 0.48 * warp * s.depth);
          var rotate = clamp(-14, 14, (scrollDelta * 0.08 + pImX * 0.05) * s.depth);
          s.setCss({
    ...
    function initCurtain() {
      ...
      if (!stage) {
        ...
        stage.innerHTML = '<div class="clt-curtain-stage__panel is-left"></div><div class="clt-curtain-stage__panel is-right"></div>';
      ...
      function raise() {
        ...
        gsap
          ...
          .to(left, { xPercent: -104, rotation: -2, duration: dur, ease: 'power2.inOut' }, 0)
          .to(right, { xPercent: 104, rotation: 2, duration: dur, ease: 'power2.inOut' }, 0.06);
      }
      function fall(done) {
        // cover, then run done()
        stage.hidden = false;
      ...
      function shouldIntercept(a, e) {
        if (!a || !a.href || e.defaultPrevented || e.button !== 0 || e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return false;
    ...
    function initReveal() {
      ...
      var els = $all('[data-clt-reveal], [data-reveal], [data-gsap~="clt-reveal"], [data-gsap~="clt-rise"]');
    ...
    function initSplit(gsap, ST, start, stagger, duration, ease, strike, replay) {
      ...
      els.forEach(function (el) {
        ...
        function units() {
          return split.words && split.words.length ? split.words : split.chars && split.chars.length ? split.chars : split.lines;
        }
        function build() {
          ...
          gsap.set(u, assembled ? { yPercent: 0, opacity: 1 } : { yPercent: 100, opacity: 0 });
        }
        build();
        if (document.fonts && document.fonts.ready) {
          document.fonts.ready
    ...
    function initLamp() {
      ...
      function getLampTrigger(el) {
        return el.closest('[data-clt-lamp-trigger]') || el.closest('.clt-footer, .clt-panel, section') || el.parentElement || el;
      }
