/* CLT core — curtain, smooth scroll, reveals, dialogs, lighting, section-nav spy. */
(function () {
  "use strict";
  ...
  // Every page carries the stage markup (Curtain Stage component, no `hidden`),
  // so CSS draws it shut from the very first paint — the page never shows
  // before the curtain covers it. Here: open it once the page is ready, and
  // close it before following an internal link. A page without the stage
  ...
  function initCurtain() {
    ...
    var left = panels[0],
      right = panels[1];
    stage.classList.add("is-live"); // switches off the CSS failsafe
    if (!left || !right || env.reducedMotion || !gsap) {
      stage.hidden = true;
      return;
    }

    var cfg = config.curtain || {};
    var openDur = cfg.duration || 1.1;
    var closeDur = cfg.closeDuration || 0.8;
    var tl = null,
      leaving = false;

    gsap.set([left, right], { transformOrigin: "50% 0%", force3D: true });

    function open() {
      if (tl) tl.kill();
      stage.hidden = false;
      gsap.set([left, right], { xPercent: 0, rotation: 0 });
      tl = gsap
        .timeline({
          onComplete: function () {
            stage.hidden = true;
        ...
        .to(left, { xPercent: -104, rotation: -2, duration: openDur, ease: CLT.motion.easeVelvet }, 0)
        .to(right, { xPercent: 104, rotation: 2, duration: openDur, ease: CLT.motion.easeVelvet }, 0.06);
    }

    function close(done) {
      if (tl) tl.kill();
      stage.hidden = false;
      gsap.set(left, { xPercent: -104, rotation: -2 });
      gsap.set(right, { xPercent: 104, rotation: 2 });
      tl = gsap
        .timeline({ onComplete: done })
        .to(left, { xPercent: 0, rotation: 0, duration: closeDur, ease: "power3.inOut" }, 0)
        .to(right, { xPercent: 0, rotation: 0, duration: closeDur, ease: "power3.inOut" }, 0.04);
    }

    // Open only once every deferred page script has run (DOMContentLoaded),
    // the fonts are in (capped) and two frames have painted underneath — so
    // the opening never stutters against page setup or reveals a relayout.
    function whenReady(cb) {
      ...
      function go() {
        ...
        if (document.fonts && document.fonts.ready) document.fonts.ready.then(frames, frames);
        setTimeout(frames, 450);
      }
      ...
      setTimeout(go, 1500); // never hold the curtain on a slow asset
    }
    whenReady(open);
    ...
    function shouldIntercept(a, e) {
      ...
      if (a.hasAttribute("data-open-dialog") || a.closest("[data-no-curtain]")) return false;
      var href = a.getAttribute("href");
      if (!href || href.charAt(0) === "#" || /^(mailto|tel|sms|javascript):/i.test(href)) return false;
      var url;
      try {
      ...
      if (/\.(pdf|zip|jpe?g|png|webp|avif|gif|mp4|webm|mov|mp3|docx?)$/i.test(url.pathname)) return false;
      return true;
    }
    ...
    document.addEventListener("click", function (e) {
      ...
      leaving = true;
      var href = a.href;
      // The mobile nav sheet (a popover) and open dialogs sit in the top layer,
      // above any z-index — close them or the curtain falls behind them.
      try {
        $all(":popover-open").forEach(function (p) { p.hidePopover(); });
      } catch (_) {}
      ...
      close(go);
      setTimeout(go, closeDur * 1000 + 400); // fail-open
    });

    // Back/forward cache: the page comes back exactly as it was left — shut.
  ...
  function initArrival() {
    // A page with a curtain opens on the curtain itself — no second fade.
    if (!document.body || !document.body.hasAttribute("data-clt-arrival")) return;
    if ($(".clt-curtain-stage")) return;
    var gsap = window.gsap;
    if (env.reducedMotion || !gsap) return;
